...
# 1. 
spear = read_excel("C:/practicavir4/spearheads.xlsx")
...
names(spear)[names(spear) == "Socle"] = "Lonitud_encaje"
...
spear$Conservacion=factor(spear$Conservacion, levels=c(1,2,3,4), labels=c("Excelente", "Buena", "Regular", "Malo"))
...
# 9.
bar_mat = barplot(table(spear$Materiales), horiz = TRUE)

bar_rem = barplot(table(spear$Remache), horiz = TRUE)
